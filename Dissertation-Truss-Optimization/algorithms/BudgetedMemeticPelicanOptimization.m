...
isDiscreteSizing = isprop(problem, 'decisionType') && ...
    strcmpi(problem.decisionType, 'discrete');
if isMixedTopology
...
objectiveValues = zeros(params.popSize, 1);
violationValues = zeros(params.popSize, 1);
...
for i = 1:params.popSize
    ...
    objectiveValues(i) = f;
    violationValues(i) = g;
    populationInfo{i} = candidateInfo;
end

[bestFit, bestIdx] = min(fitness);
bestSol = pelicans(bestIdx, :);
bestObjective = objectiveValues(bestIdx);
bestViolation = violationValues(bestIdx);
bestInfo = populationInfo{bestIdx};
...
while evaluationCount < params.maxEvaluations
    ...
    for i = 1:params.popSize
        ...
        if candFit < fitness(i)
            ...
            fitness(i) = candFit;
            objectiveValues(i) = candF;
            violationValues(i) = candG;
            populationInfo{i} = candInfo;
            if candFit < bestFit
                bestFit = candFit;
                bestSol = candidate;
                bestObjective = candF;
    ...
    for trial = 1:params.localSearchTrials
        ...
        if isDiscreteSizing && ~isMixedTopology
    ...
    if params.restartFraction > 0 && ...
        ...
        for k = 1:numel(restartIdx)
            ...
            fitness(idx) = candFit;
            objectiveValues(idx) = candF;
            violationValues(idx) = candG;
            populationInfo{idx} = candInfo;
            if candFit < bestFit
                bestFit = candFit;
                bestSol = candidate;
                bestObjective = candF;
...
% One evaluator call may solve several independent load cases.
