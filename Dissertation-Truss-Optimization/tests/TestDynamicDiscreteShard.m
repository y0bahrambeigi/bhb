...
result = RunDynamicDiscreteShard('72-bar','qio',84,2026);
assert(size(result.rows,1) == 1,'Shard smoke run must return one row.');
assert(result.rows{1,7} == 84,'Shard must preserve exact evaluator budget.');
assert(result.rows{1,9} == 84,'Every dynamic evaluator call must include one modal solve.');
assert(result.rows{1,10} > 0,'QIO shard must execute QIO attempts.');

csvPath = fullfile(outDir,'72_bar__qio__FE84__seeds2026_2026.csv');
matPath = fullfile(outDir,'72_bar__qio__FE84__seeds2026_2026.mat');
