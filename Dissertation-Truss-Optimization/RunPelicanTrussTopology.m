...
memberRows = [num2cell(member), num2cell(nodeI), num2cell(nodeJ), ...
    num2cell(active), num2cell(area), num2cell(stress), num2cell(stressRatio)];
memberResults = MakeResultSet(memberRows, ...
    {'Member', 'NodeI', 'NodeJ', 'Active', 'Area_in2', 'Stress_ksi', 'StressRatio'});

disp(memberResults.rows);
WriteResultCsv(fullfile(outputDir, 'member_results.csv'), memberResults);

summaryRows = {objective, bestFit, violation, info.nActive, ...
    info.maxStress, info.maxDisp, info.conditionEstimate, info.isStable, ...
    info.isFeasible, elapsedTime};
summaryResults = MakeResultSet(summaryRows, ...
    {'Weight_lb', 'PenalizedFitness', 'ConstraintViolation', ...
     'ActiveBars', 'MaxStress_ksi', 'MaxDisp_in', ...
     'ReciprocalCondition', 'Stable', 'Feasible', 'ElapsedTime_s'});
WriteResultCsv(fullfile(outputDir, 'summary_results.csv'), summaryResults);
