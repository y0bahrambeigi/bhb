# Figshare metadata — MohandesYar AI 2.0 Technical Report

This file records the public archival metadata for the technical report as a
research object distinct from the MohandesYar AI 2.0.0 software archive.

- **Figshare item ID:** 33935692
- **Resource type:** Preprint / technical report
- **Title:** MohandesYar AI 2.0: An Offline-First Persian PWA for Civil Engineering Field Documentation, Evidence Integrity, and Reporting
- **Creator:** Yousef Bahrambeigi
- **Canonical Figshare author ID:** 24754576
- **Affiliation:** Civil Engineering, Islamic Azad University, Mahabad Branch, Mahabad, Iran
- **ORCID:** 0000-0002-3421-8679
- **Report number:** MYAI-TR-2026-02
- **First public availability stated in report:** 2026-08-25
- **Initial Figshare publication timestamp:** 2026-09-18T13:14:31Z
- **Canonical metadata version:** 2 (author metadata normalized to one canonical Figshare author)
- **License:** CC BY 4.0
- **Category:** Structural engineering (Figshare category 26371)
- **Primary file:** `mohandesyar-ai-v2-technical-report.pdf`
- **PDF SHA-256:** `c938de488c8b1e70eb8c7a53c38f30d5e719effda6256b5599f18c5ae8453e16`
- **Concept DOI:** `10.6084/m9.figshare.33935692`
- **Version DOI (canonical):** `10.6084/m9.figshare.33935692.v2`
- **Version 1 DOI (historical):** `10.6084/m9.figshare.33935692.v1`
- **DOI URL:** https://doi.org/10.6084/m9.figshare.33935692.v2
- **Public API record:** https://api.figshare.com/v2/articles/33935692
- **Related software DOI:** `10.6084/m9.figshare.33511795.v1`
- **Related software item:** 33511795
- **Report landing page:** https://y0bahrambeigi.github.io/bhb/mohandesyar-ai/publication/
- **Source repository:** https://github.com/y0bahrambeigi/bhb/tree/main/mohandesyar-ai

## Integrity and identity

The technical-report PDF was SHA-256 verified immediately before upload. The
report DOI identifies the scholarly document; the separate software DOI
`10.6084/m9.figshare.33511795.v1` identifies the frozen MohandesYar AI 2.0.0 software archive.
These identifiers must not be interchanged in citation metadata.

## Zenodo note

A previously reserved Zenodo DOI `10.5281/zenodo.22089146` was not published.
It is not an active DOI for this report and must not be cited. If the exact same
report is later preserved on Zenodo, use the existing Figshare DOI rather than
minting a duplicate DOI for the identical research object.
